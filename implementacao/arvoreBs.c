...
static bool pesquisaBinaria(Registro *pagina, int tamanho, int chave, Registro *reg_retorno)
{
...
static bool pesquisa(Pagina_ *raiz, int chave, Registro *reg_retorno)
{
    short int i;

    if(raiz->tp == FOLHA)
        return pesquisaBinaria(raiz->Uniao.Folha.registros, raiz->Uniao.Folha.n, chave, reg_retorno);
    ...
    if(chave < raiz->Uniao.Indice.indices[i - 1])
        return arvoreB_(raiz->Uniao.Indice.prox_pagina[i - 1], chave, reg_retorno);

    return arvoreB_(raiz->Uniao.Indice.prox_pagina[i], chave, reg_retorno);
}
...
bool arvoreB_(Pagina_ *raiz, int chave, Registro *reg_retorno)
{
    return pesquisa(raiz, chave, reg_retorno);
}