...
void montarArvore(FILE *arqBase, FILE *arqDestino, int numDeRegistros) {
    No newItem;
    Registro raiz;
    ...
    inserirItem(arqDestino, newItem, 0,  1);
    ...
    for (int i = 1; i < numDeRegistros; i++) {
        ...
        newItem.esq = newItem.dir = -1;
        inserirItem(arqDestino, newItem, 0, 0);
    }
}


// arquivo p inserir -> item a ser inserido -> posicao do itemReferencia (itemAtual) -> bool: raiz ou nao
void inserirItem(FILE *arqDestino, No newItem, long int deslocamentoReferencia, short flagRaiz) {
    if (flagRaiz ==  1) {
    ...
    if (newItem.registro.chave < itemReferencia.registro.chave) { // insercao aa esquerda
        ...
        inserirItem(arqDestino, newItem, itemReferencia.esq, 0);
        ...
        inserirItem(arqDestino, newItem, itemReferencia.dir, 0);
    } 
} 

bool findChave(FILE* arqDestino, long int root_offset, int chaveAlvo, Registro* reg_retorno) {
    // [ATINGIU UM NÓ FOLHA]
    if(root_offset == -1){ 
        return false;
    }
    ...
    if(chaveAlvo == itemAtual.registro.chave){
        *reg_retorno = itemAtual.registro;
        return true;
    } else if (chaveAlvo < itemAtual.registro.chave){
        return findChave(arqDestino, itemAtual.esq, chaveAlvo, reg_retorno);
    } else {
        return findChave(arqDestino, itemAtual.dir, chaveAlvo, reg_retorno);
    }
}
